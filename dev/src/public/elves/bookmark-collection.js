...
const link = elf('bookmark-collection', {
  schedule: {},
  sessions: [],
  form: {},
...
link.draw((target) => {
  ...
  return `
    <div style="text-align: center;">
      <a href="/app/link-list" class="nonce" data-tooltip="home" aria-label="home"></a>
    </div>
    <div class="item">
    ...
    <div class="item">
      <button class="head" data-accordion="sheet">
        <a href="" class="nonce"></a>
        Sheet
      </button>
      <div class="body">
      ...
      if(data && sheet) {
        jspreadsheet(sheet, {
          ...
          columns:[
            ...
            { title:'type', width:80 },
            { title:'who', width:120 },
            { title:'what', width:300 },
            { title:'why', width:300 },
            { title:'timestamp', width:300 },
          ]
        });
      }
...
async function fetchPerformers(target) {
  try {
    ...
    link.teach({ error: error.toString(), key: null });
...
link.when('submit', 'form', async (event) => {
  ...
  if(form.who && form.when && form.why && form.what) {
    ...
    try {
      const response = await fetch(plan98.env.PLAN98_API_HOST + '/api/performer', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({...form, where: '/app/middle-earth' }),
      });
